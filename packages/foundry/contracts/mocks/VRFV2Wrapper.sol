...
import { ChainSpecificUtil } from "./dependencies/ChainSpecificUtil.sol";
...
contract VRFV2Wrapper is ConfirmedOwner, TypeAndVersionInterface, VRFConsumerBaseV2, VRFV2WrapperInterface {
    ...
    function _calculateRequestPrice(uint256 _gas, uint256 _requestGasPrice, int256 _weiPerUnitLink)
        internal
        view
        returns (uint256)
    {
     ...
     */
    function onTokenTransfer(address _sender, uint256 _amount, bytes calldata _data)
        external
        onlyConfiguredNotDisabled
    {
